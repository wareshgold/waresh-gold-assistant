...
**Protected GitHub tag:** `checkpoint/ai-sp2l-vip-2026-08-22`
**Baseline branch:** `feature/vip-foundation`
**Current work branch:** `fix/vip-sp2l-production-readiness`
...
The known-good baseline is commit `a2fc0b8`, protected by the GitHub tag `checkpoint/ai-sp2l-vip-2026-08-22`.
...
13. Created the new continuation branch `fix/vip-sp2l-production-readiness` from the protected checkpoint.
...
Application
  ...
  └── SP2L strategy/use cases/jobs
        ↓
Domain
  ├── Gold rules/calculations
  ├── VIP model
  └── SP2L signal engine/model
        ↓
Infrastructure
...
3. **SP2L scheduler wiring** — verify and deliberately wire `SP2LSignalSchedulerJob` only after defining cadence and execution semantics.
4. **SP2L publication idempotency** — prevent duplicate publication/notification of the same actionable signal before enabling automated scheduling.
...
7. Evaluate durable/auditable persistence for SP2L signals.
...
10. Review AI ↔ SP2L/VIP boundaries for accidental coupling.

## Planned path forward

1. Keep `checkpoint/ai-sp2l-vip-2026-08-22` immutable as the rollback point.
2. Work only on `fix/vip-sp2l-production-readiness` for the next changes.
...
```powershell
git switch --detach checkpoint/ai-sp2l-vip-2026-08-22
```

4. Protected baseline commit: `a2fc0b8`.
5. Active continuation branch: `fix/vip-sp2l-production-readiness`.
...
- GitHub tag `checkpoint/ai-sp2l-vip-2026-08-22`
- commit `a2fc0b8`
- this recovery document
- current continuation branch `fix/vip-sp2l-production-readiness`

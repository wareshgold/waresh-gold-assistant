...
export function createApp(container: AppContainer) {
  ...
  app.get("/system/metrics", async (c) => {
    const metrics = await container.systemMetricsController.handle();
  ...
  app.get("/market/history", async (c) => {
    ...
    const history = await container.snapshotService.getHistory(limit);
  ...
  app.post("/api/v1/calculate/gold-price", async (c) => {
    ...
    if (!body) {
      return c.json({ error: "درخواست نامعتبر است." }, 400);
    }
    ...
    const result = container.calculateGoldPriceUseCase.execute({
      weight,
      goldPrice,
      laborPercent,
      profitPercent,
  ...
  app.post("/telegram/webhook", async (c) => {
    return await container.telegramWebhookController.handle(c);
  });

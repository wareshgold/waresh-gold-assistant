...
export class TwoLegDetector {
    detect(candles: SP2LCandle[], spike: Spike, config: SP2LConfiguration): TwoLeg | null {
        const afterStart = spike.endIndex + 1;
        if (afterStart >= candles.length - 2) return null;

        const correction = candles.slice(afterStart);
        if (correction.length < 3) return null;
    ...
    private detectBullishCorrection(candles: SP2LCandle[], spike: Spike, fromIndex: number, config: SP2LConfiguration): TwoLeg | null {
        ...
        const leg2: SwingLeg = {
            ...
            endIndex: leg2End,
            startPrice: candles[bounceEnd].high,
            endPrice: candles[leg2End].low
        };

        return this.buildTwoLeg(spike, leg1, leg2, config);
    }

    private detectBearishCorrection(candles: SP2LCandle[], spike: Spike, fromIndex: number, config: SP2LConfiguration): TwoLeg | null {
        ...
        const leg2: SwingLeg = {
            startIndex: dipEnd,
            endIndex: leg2End,
            startPrice: candles[dipEnd].low,
            endPrice: candles[leg2End].high
        };

        return this.buildTwoLeg(spike, leg1, leg2, config);
    }

    private buildTwoLeg(spike: Spike, leg1: SwingLeg, leg2: SwingLeg, config: SP2LConfiguration): TwoLeg | null {
    ...
    private findSwingLowIndex(candles: SP2LCandle[], from: number, to: number): number | null {
        if (from > to || from >= candles.length) return null;
        let minIdx = from;
        let minVal = candles[from].low;
        for (let i = from; i <= Math.min(to, candles.length - 1); i++) {
    ...
    private findSwingHighIndex(candles: SP2LCandle[], from: number, to: number): number | null {
        if (from > to || from >= candles.length) return null;
        let maxIdx = from;

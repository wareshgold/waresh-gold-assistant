...
export class TwoLegDetector {
    ...
    private detectBullishCorrection(candles: StrategyACandle[], spike: Spike, fromIndex: number, config: StrategyAConfiguration): TwoLeg | null {
        ...
        for (let i = fromIndex; i < candles.length - 2; i++) {
            const previous = candles[i - 1]; const current = candles[i]; const next = candles[i + 1];
            if (current.low < previous.low && (next.high > current.high || next.close > current.close)) { leg1End = i; break; }
        }
        ...
        for (let i = leg1End + 1; i < candles.length - 1; i++) {
            if (candles[i].high > candles[leg1End].high || candles[i].close > candles[leg1End].close) { retracementIndex = i; break; }
        }
        if (retracementIndex === null) return null;
        let completionIndex: number | null = null;
        for (let i = retracementIndex + 1; i < candles.length - 1; i++) {
            if (candles[i].low < candles[leg1End].low) { completionIndex = i; break; }
        }
        if (completionIndex === null) return null;
        const confirmationIndex = completionIndex + 1;
        ...
        return this.buildTwoLeg(spike, leg1, leg2, candles[completionIndex].close, confirmationIndex, config);
    }
    ...
    private detectBearishCorrection(candles: StrategyACandle[], spike: Spike, fromIndex: number, config: StrategyAConfiguration): TwoLeg | null {
        ...
        for (let i = fromIndex; i < candles.length - 2; i++) {
            const previous = candles[i - 1]; const current = candles[i]; const next = candles[i + 1];
            if (current.high > previous.high && (next.low < current.low || next.close < current.close)) { leg1End = i; break; }
        }
        if (leg1End === null) return null;
        const leg1: SwingLeg = { startIndex: fromIndex, endIndex: leg1End, startPrice: candles[fromIndex].low, endPrice: candles[leg1End].high };
        let retracementIndex: number | null = null;
        for (let i = leg1End + 1; i < candles.length - 1; i++) {
            if (candles[i].low < candles[leg1End].low || candles[i].close < candles[leg1End].close) { retracementIndex = i; break; }
        }
        if (retracementIndex === null) return null;
        let completionIndex: number | null = null;
        for (let i = retracementIndex + 1; i < candles.length - 1; i++) {
            if (candles[i].high > candles[leg1End].high) { completionIndex = i; break; }
        }
        if (completionIndex === null) return null;
        const confirmationIndex = completionIndex + 1;
        const confirmation = candles[confirmationIndex];
    ...
    private buildTwoLeg(spike: Spike, leg1: SwingLeg, leg2: SwingLeg, completionPrice: number, confirmationIndex: number, config: StrategyAConfiguration): TwoLeg | null {

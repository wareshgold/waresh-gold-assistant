...
const forestImage = "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&w=1800&q=85";
  ...
  return (
    <ScrollExperience>
      <main className="waresh-site overflow-x-hidden bg-[#f5f1e9] text-[#292b26]">
        ...
        <section id="top" className="waresh-scroll-scene relative min-h-[720px] overflow-hidden bg-[#20372c] text-white">
          <img src={forestImage} alt="" className="waresh-scene-image absolute inset-[-2%] h-[104%] w-[104%] object-cover opacity-45" aria-hidden="true" data-waresh-parallax="18" />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(20,39,31,0.98)_0%,rgba(24,52,40,0.80)_42%,rgba(20,42,34,0.35)_100%)]" />
          <div className="waresh-rain absolute inset-0 opacity-20" aria-hidden="true" />
          <div className="waresh-container relative grid min-h-[720px] items-center gap-14 py-20 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="waresh-hero-copy max-w-2xl">
              <p className="waresh-reveal text-xs font-bold tracking-[0.24em] text-[#d7bf8b]" data-reveal="up">WARESH GOLD · JEWELRY</p>
              <h1 className="waresh-reveal mt-6 text-5xl font-extrabold leading-[1.35] tracking-tight sm:text-7xl" data-reveal="up" data-delay="1">طلای خاص،<span className="block text-[#d8bd7e]">برای لحظه‌های خاص.</span></h1>
              <p className="waresh-reveal mt-7 max-w-xl text-base leading-9 text-[#d9e0da] sm:text-lg" data-reveal="up" data-delay="2">انتخابی از طلا و جواهرات ظریف، کم‌اجرت و ماندگار؛ با قیمت بازار و ابزارهای دقیق وارش گلد.</p>
              <div className="waresh-reveal mt-9 flex flex-col gap-3 sm:flex-row" data-reveal="up" data-delay="3"><a href="#products" className="inline-flex items-center justify-center rounded-full bg-[#b28b4c] px-7 py-4 font-bold text-white transition hover:-translate-y-0.5 hover:bg-[#9d773d]">مشاهده محصولات</a><a href="#prices" className="inline-flex items-center justify-center rounded-full border border-white/25 bg-white/10 px-7 py-4 font-bold text-white backdrop-blur-sm transition hover:bg-white/15">قیمت امروز</a></div>
              <div className="waresh-reveal mt-12 flex flex-wrap gap-x-8 gap-y-3 text-xs text-[#c7d0c8]" data-reveal="up" data-delay="4"><span>محصولات منتخب</span><span>قیمت شفاف</span><span>محاسبه دقیق</span></div>
            </div>
            <div className="waresh-reveal relative mx-auto w-full max-w-[440px] lg:justify-self-end" data-reveal="right" data-delay="2">
              <div className="waresh-parallax overflow-hidden rounded-[3rem] border border-white/15 bg-white/10 p-2 shadow-[0_40px_100px_rgba(0,0,0,0.3)] backdrop-blur-sm" data-waresh-parallax="12"><div className="relative overflow-hidden rounded-[2.6rem]"><img src={jewelryImage} alt="نمونه‌ای از طلا و جواهرات وارش" className="h-[500px] w-full object-cover" /><div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_45%,rgba(20,39,31,0.82)_100%)]" /><div className="absolute inset-x-7 bottom-7"><p className="text-[10px] font-bold tracking-[0.22em] text-[#dbc58d]">SELECTED GOLD</p><p className="mt-2 text-2xl font-extrabold">زیبایی، بدون هیاهو.</p></div></div></div>
            </div>
          </div>
        ...
        <section id="gifts" className="waresh-scroll-scene relative overflow-hidden bg-[#23372d] py-24 text-white sm:py-28"><div className="waresh-container relative"><div className="waresh-reveal max-w-2xl" data-reveal="right"><p className="text-xs font-bold tracking-[0.2em] text-[#d8bd7e]">GIFT EDIT</p><h2 className="mt-5 text-4xl font-extrabold sm:text-5xl">هدیه‌ای که می‌ماند.</h2><p className="mt-5 text-base leading-8 text-[#d6ddd6]">برای انتخاب هدیه، از فیلتر بازه قیمت در مجموعه بالا استفاده کن؛ انتخاب بر اساس بودجه، ساده و شفاف.</p></div><div className="mt-12 grid gap-4 sm:grid-cols-3">{[["۳ تا ۱۰ میلیون","انتخاب‌های ظریف و اقتصادی"],["۱۰ تا ۲۰ میلیون","برای یک هدیه ویژه"],["۲۰ تا ۳۰ میلیون","انتخاب‌های چشمگیرتر"]].map(([range,description], index)=><a key={range} href="#products" className="waresh-reveal group rounded-[2rem] border border-white/10 bg-white/[0.06] p-7 backdrop-blur transition hover:-translate-y-1 hover:bg-white/[0.1]" data-reveal="up" data-delay={String(index + 1)}><span className="text-sm font-bold text-[#dbc58d]">{range}</span><p className="mt-5 text-xl font-extrabold">{description}</p><span className="mt-8 block text-sm text-[#bfc9c0] transition group-hover:text-white">دیدن مجموعه ←</span></a>)}</div></div></section>

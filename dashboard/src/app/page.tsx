  ...
  return (
    <ScrollExperience>
      <main className="waresh-site overflow-x-hidden bg-[#f5f1e9] text-[#292b26]">
        ...
        <section id="top" className="waresh-scroll-scene relative min-h-[620px] overflow-hidden bg-[#182b24] text-white sm:min-h-[660px]">
          ...
          <div className="waresh-container relative grid min-h-[620px] items-center gap-12 py-14 sm:min-h-[660px] sm:py-16 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="max-w-2xl">
              ...
              <p className="mt-6 max-w-xl text-base leading-9 text-[#d9e0da] sm:text-lg">مدل‌های ظریف و اقتصادی، قیمت شفاف و محاسبه دقیق؛ تا انتخاب طلا از یک خرید، به یک انتخاب مطمئن تبدیل شود.</p>
              ...
              <div className="mt-9 flex flex-wrap gap-x-8 gap-y-3 text-xs text-[#c7d0c8]"><span>انتخاب اقتصادی</span><span>قیمت شفاف</span><span>محاسبه دقیق</span></div>
            </div>
        ...
        <section id="prices" className="waresh-scroll-scene bg-[#e9eee7] py-24 sm:py-28"><div className="waresh-container"><div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start"><div className="waresh-reveal lg:sticky lg:top-28" data-reveal="right"><div className="flex flex-wrap items-center gap-3"><p className="text-xs font-bold tracking-[0.2em] text-[#9b7843]">MARKET / LIVE</p><span className="inline-flex items-center gap-1.5 rounded-full border border-[#cfdacb] bg-[#f7faf5] px-2.5 py-1 text-[10px] font-bold text-[#5f755f]"><span className="h-1.5 w-1.5 rounded-full bg-[#6f9b72]" aria-hidden="true" />اتصال به بازار</span></div><h2 className="mt-4 text-4xl font-extrabold text-[#293129] sm:text-5xl">نبض بازار</h2><p className="mt-5 max-w-lg text-base leading-8 text-[#70776e]">قیمت‌های بازار از هسته اصلی وارش دریافت می‌شوند؛ سایت فقط آن‌ها را نمایش می‌دهد و محاسبات را به سرویس مطمئن طلا می‌سپارد.</p><div className="mt-8 grid grid-cols-2 gap-3 sm:max-w-md">{[["طلای ۱۸ عیار",market?formatToman(market.gold18Price):"—"],["دلار تهران",market?formatToman(market.currencyPrice):"—"],["انس جهانی",market?Number(market.ouncePrice).toLocaleString("en-US"):"—"],["حباب",bubble?formatToman(bubble.bubbleAmount):"—"]].map(([label,value])=><div key={label} className="rounded-[1.5rem] bg-[#faf8f2] p-5"><p className="text-xs text-[#858a83]">{label}</p><p className="mt-2 text-xl font-extrabold text-[#9b753c]">{value}</p></div>)}</div><p className="mt-4 text-xs leading-6 text-[#858a83]">اعداد این بخش برای نمایش وضعیت فعلی بازار هستند؛ محاسبه نهایی از سرویس مرکزی انجام می‌شود.</p></div><div className="waresh-reveal" data-reveal="left" data-delay="1"><GoldCalculator liveGoldPrice={market?.gold18Price} /></div></div></div></section>

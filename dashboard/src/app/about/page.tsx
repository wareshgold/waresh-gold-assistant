...
export default function AboutPage() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f5f1e9] text-[#292b26]">
      <header className="sticky top-0 z-50 border-b border-[#dedfd7]/80 bg-[#faf8f2]/95 backdrop-blur-xl">
        <div className="waresh-container flex h-[76px] items-center justify-between gap-4 sm:gap-5">
          <Link href="/" aria-label="بازگشت به خانه" className="shrink-0">
            <img src="/waresh-gold-logo-green.png" alt="وارش گلد" width={180} height={48} className="h-11 w-auto object-contain sm:h-12" />
          </Link>
      ...
      <footer className="bg-[#1f2d26] py-10 text-white sm:py-12"><div className="waresh-container flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between"><div><img src="/waresh-gold-logo-white.jpg" alt="وارش گلد" width={180} height={44} loading="lazy" decoding="async" className="h-10 w-auto object-contain sm:h-11" /><p className="mt-3 max-w-sm text-xs leading-6 text-white/45">فروشگاه طلا و ابزارهای دقیق وارش؛ ریشه شمالی، نگاه رو به آینده.</p></div><div className="flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/65"><Link className="transition hover:text-white" href="/">فروشگاه</Link><Link className="transition hover:text-white" href="/tools">ابزار طلا</Link><Link className="transition hover:text-white" href="/about">درباره وارش</Link><a className="transition hover:text-white" href={TELEGRAM_BOT_URL} target="_blank" rel="noopener noreferrer">تلگرام</a></div></div></footer>
    </main>
  );
}

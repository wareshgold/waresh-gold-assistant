...
export async function POST(request: NextRequest) {
  ...
  if (!["username", "password", "phone", "nationalId"].every((key) => typeof input[key] === "string")) return NextResponse.json({ error: "اطلاعات ثبت‌نام کامل نیست." }, { status: 400 });
  ...
  try {
    ...
    if (!response.ok || !data?.sessionId) return NextResponse.json(data ?? { error: "ثبت‌نام انجام نشد." }, { status: response.status || 400, headers: { "Cache-Control": "no-store" } });

    const next = NextResponse.json({ customer: data.customer }, { status: 201, headers: { "Cache-Control": "no-store" } });

...
export default function CartPage() {
  ...
  const missingCartItems = !loadingProducts && cart.length !== products.length;
  ...
  useEffect(() => {
    ...
    const invalidItems = cart.filter((item) => !validKeys.has(`${item.productId}:${item.variantId}`));
    if (invalidItems.length) {
      setCart((current) => current.filter((item) => validKeys.has(`${item.productId}:${item.variantId}`)));
    }

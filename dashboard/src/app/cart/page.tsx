...
import { CART_CHANGE_EVENT, getCartCount, readCart, removeFromCart, updateCartQuantity, type CartItem } from "@/lib/cart";
      ...
      <section className="waresh-container py-10 sm:py-16">
        ...
        {!products.length ? (
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-[1fr_360px] lg:items-start">
            ...
            <aside className="lg:sticky lg:top-28 rounded-[1.75rem] border border-[#ded8cc] bg-[#fffdf8] p-5 shadow-[0_18px_50px_rgba(55,52,43,0.06)] sm:p-6"><p className="text-xs font-bold text-[#929188]">خلاصه سفارش</p><div className="mt-5 flex items-center justify-between text-sm"><span className="text-[#777970]">تعداد</span><strong>{count} عدد</strong></div><div className="mt-3 flex items-center justify-between text-sm"><span className="text-[#777970]">جمع فعلی</span><strong className="text-[#9b753c]">{total > 0 ? formatToman(total) : "—"}</strong></div>{priceError && <p className="mt-4 rounded-2xl bg-[#f8eee9] px-3 py-2 text-[11px] leading-6 text-[#94675f]">قیمت بعضی محصولات در حال حاضر در دسترس نیست؛ برای ثبت سفارش باید نرخ بازار دوباره دریافت شود.</p>}<div className="mt-5 border-t border-[#e6e0d5] pt-5 text-[11px] leading-6 text-[#88877f]">قیمت نهایی هنگام ادامه فرایند خرید دوباره با نرخ بازار بررسی خواهد شد.</div><Link href={total > 0 && !priceError ? "/checkout" : "#"} aria-disabled={total <= 0 || priceError} onClick={(event) => { if (total <= 0 || priceError) event.preventDefault(); }} className="mt-5 flex min-h-12 w-full items-center justify-center rounded-full bg-[#25392f] px-5 py-3.5 text-sm font-bold text-white shadow-[0_14px_35px_rgba(37,57,47,0.14)] transition hover:-translate-y-0.5 hover:bg-[#1d3028] aria-disabled:pointer-events-none aria-disabled:opacity-45">ادامه و ثبت سفارش</Link><Link href="/#products" className="mt-3 flex min-h-11 items-center justify-center rounded-full border border-[#ded8cc] bg-white px-5 text-xs font-bold text-[#62685e]">بازگشت به محصولات</Link></aside>
          </div>
        )}
      </section>

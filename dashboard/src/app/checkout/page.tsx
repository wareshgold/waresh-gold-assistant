...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const sync = () => { setCart(readCart()); setSubmitted(false); setQuote(null); setOrder(null); setOrderError(false); };
    sync();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setValidatingOrder(true); setSubmitted(false); setQuote(null); setOrder(null); setOrderError(false);
    try {
      const isFresh = await refreshPrices(); if (!isFresh) return;
  ...
  const handleCreateOrder = async () => {
    ...
    if (!customer) { setOrderError(true); return; }
    if (!selectedAddressId) { setOrderError(true); return; }
    setCreatingOrder(true); setOrderError(false);
    try {

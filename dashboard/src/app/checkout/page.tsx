...
function quoteMatchesCart(quoteItems: CheckoutQuote["items"], cartItems: CartItem[]) {
  ...
  const quoteKeys = quoteItems
    .map((item) => `${item.productId}::${item.variantId}::${item.quantity}`)
    .sort();
  const cartKeys = cartItems
    .map((item) => `${String(item.productId)}::${item.variantId}::${item.quantity}`)
    .sort();

    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f5f1e9] text-[#292b26]">
      <header className="sticky top-0 z-50 border-b border-[#dedfd7]/80 bg-[#faf8f2]/95 backdrop-blur-xl">
        <div className="waresh-container flex h-[76px] items-center justify-between gap-4 sm:gap-5">
          ...
          <div className="flex items-center gap-2"><a href={TELEGRAM_BOT_URL} target="_blank" rel="noopener noreferrer" className="hidden min-h-11 items-center rounded-full bg-[#b28b4c] px-5 py-2.5 text-xs font-bold text-white transition hover:-translate-y-0.5 hover:bg-[#9d773d] sm:inline-flex">مشاوره و سفارش</a><MobileMenu /></div>
        </div>
      </header>
      ...
      <section className="waresh-container py-8 sm:py-12 lg:py-16">
        <div className="grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:gap-12">
          <div className="grid gap-3 sm:grid-cols-[96px_1fr] sm:gap-4">
            <div className="order-2 grid grid-cols-2 gap-3 sm:order-1 sm:grid-cols-1">{gallery.map((image, index) => <div key={`${image}-${index}`} className="overflow-hidden rounded-[1.25rem] border border-[#e0dbd1] bg-[#eee8dc]"><img src={image} alt={index === 0 ? product.name : `${product.name} نمای نزدیک`} className="aspect-square h-full w-full object-cover" loading={index === 0 ? "eager" : "lazy"} referrerPolicy="no-referrer" /></div>)}</div>
            <div className="order-1 overflow-hidden rounded-[2rem] border border-[#e0dbd1] bg-[#eee8dc] shadow-[0_24px_70px_rgba(55,52,43,0.08)] sm:order-2 sm:rounded-[2.5rem]"><img src={product.image} alt={product.name} className="aspect-[4/5] h-full w-full object-cover" referrerPolicy="no-referrer" /></div>
          </div>
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="mt-7 rounded-[1.75rem] border border-[#ded8cc] bg-[#fffdf8] p-5 shadow-[0_18px_50px_rgba(55,52,43,0.06)] sm:p-6">
              <div className="flex items-end justify-between gap-5"><div><p className="text-xs font-semibold text-[#929188]">قیمت نهایی</p><p className="mt-2 text-2xl font-extrabold text-[#9b753c] sm:text-3xl">{price !== null ? formatToman(price) : "در حال دریافت قیمت"}</p></div><div className="text-left" dir="rtl"><p className="text-[10px] text-[#a09d94]">مبنای بازار</p><p className="mt-1 text-sm font-bold text-[#55584f]">{market ? formatToman(market.gold18Price) : "—"}</p><p className="mt-1 text-[10px] text-[#a09d94]">هر گرم طلای ۱۸ عیار</p></div></div>
              <div className="mt-5 grid grid-cols-2 gap-2 text-xs"><div className="rounded-2xl bg-[#f5f1e9] p-3"><span className="text-[#8c8b83]">وزن</span><strong className="mr-2 text-[#4f554d]">{formatWeight(product.weight)}</strong></div><div className="rounded-2xl bg-[#f5f1e9] p-3"><span className="text-[#8c8b83]">اجرت</span><strong className="mr-2 text-[#4f554d]">{product.laborPercent}٪</strong></div><div className="rounded-2xl bg-[#f5f1e9] p-3"><span className="text-[#8c8b83]">سود</span><strong className="mr-2 text-[#4f554d]">{product.profitPercent}٪</strong></div><div className="rounded-2xl bg-[#f5f1e9] p-3"><span className="text-[#8c8b83]">مالیات</span><strong className="mr-2 text-[#4f554d]">{product.taxPercent}٪</strong></div></div>
              ...
              <a href={TELEGRAM_BOT_URL} target="_blank" rel="noopener noreferrer" className="mt-5 flex min-h-12 w-full items-center justify-center rounded-full bg-[#25392f] px-5 py-3.5 text-sm font-bold text-white shadow-[0_14px_35px_rgba(37,57,47,0.16)] transition hover:-translate-y-0.5 hover:bg-[#1d3028]">مشاوره و سفارش در تلگرام</a>
            </div>

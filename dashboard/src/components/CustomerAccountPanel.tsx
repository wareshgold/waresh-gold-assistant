...
export default function CustomerAccountPanel() {
  ...
  const loadAccount = async () => {
    try {
      ...
      if (response.ok && data?.customer) setProfile(saveCustomerProfile(toProfile(data.customer)));
    ...
    return (
      <div className="rounded-[2rem] border border-[#ded8cc] bg-[#fffdf8] p-6 shadow-[0_18px_50px_rgba(55,52,43,0.06)] sm:p-8">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          ...
          <Link href="/orders" className="rounded-[1.5rem] border border-[#e3ddd2] bg-white p-5 transition hover:-translate-y-0.5"><p className="text-xs text-[#929188]">سفارش‌ها</p><p className="mt-2 text-sm font-extrabold">تاریخچه سفارش‌ها</p></Link>
        ...
        <section className="mt-8 rounded-[1.75rem] border border-[#e3ddd2] bg-white p-5 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-xs font-bold tracking-[0.16em] text-[#9b7b48]">ADDRESSES</p><h2 className="mt-2 text-lg font-extrabold">آدرس‌های ارسال</h2><p className="mt-1 text-xs leading-6 text-[#777970]">آدرس‌ها در حساب شما ذخیره می‌شوند و هنگام ثبت سفارش قابل انتخاب هستند.</p></div>
            <button type="button" onClick={() => { setShowAddressForm((value) => !value); setMessage(""); }} className="min-h-11 rounded-full bg-[#25392f] px-5 text-xs font-bold text-white">{showAddressForm ? "بستن فرم" : "افزودن آدرس"}</button>
          </div>

          {showAddressForm && <div className="mt-5 grid gap-4 border-t border-[#eee9df] pt-5 sm:grid-cols-2">
            <Field id="address-title" label="عنوان آدرس" value={addressForm.title} onChange={(value) => setAddressForm({ ...addressForm, title: value })} placeholder="خانه" disabled={addressLoading} />
            <Field id="address-recipient" label="نام گیرنده" value={addressForm.recipientName} onChange={(value) => setAddressForm({ ...addressForm, recipientName: value })} placeholder="نام و نام خانوادگی" disabled={addressLoading} />
            <Field id="address-phone" label="شماره گیرنده" value={addressForm.phone} onChange={(value) => setAddressForm({ ...addressForm, phone: value })} placeholder="۰۹۱۲..." inputMode="tel" disabled={addressLoading} />
            <Field id="address-province" label="استان" value={addressForm.province} onChange={(value) => setAddressForm({ ...addressForm, province: value })} placeholder="تهران" disabled={addressLoading} />
            <Field id="address-city" label="شهر" value={addressForm.city} onChange={(value) => setAddressForm({ ...addressForm, city: value })} placeholder="تهران" disabled={addressLoading} />
            <Field id="address-postal-code" label="کد پستی" value={addressForm.postalCode} onChange={(value) => setAddressForm({ ...addressForm, postalCode: value.replace(/\D/g, "").slice(0, 10) })} placeholder="۱۰ رقم" inputMode="numeric" disabled={addressLoading} />
            <label className="block sm:col-span-2"><span className="text-xs font-bold text-[#55584f]">نشانی کامل</span><textarea value={addressForm.address} onChange={(event) => setAddressForm({ ...addressForm, address: event.target.value })} rows={4} disabled={addressLoading} className="mt-2 w-full rounded-2xl border border-[#ddd7cb] bg-white px-4 py-3 text-sm leading-7 outline-none transition focus:border-[#b28b4c]" placeholder="خیابان، کوچه، پلاک، واحد..." /></label>
            <button type="button" onClick={() => void submitAddress()} disabled={addressLoading} className="min-h-12 rounded-full bg-[#25392f] px-5 text-sm font-bold text-white disabled:opacity-50 sm:col-span-2">{addressLoading ? "در حال ذخیره..." : "ذخیره آدرس"}</button>
          </div>}

          <div className="mt-5 space-y-3">
            {profile.addresses.length ? profile.addresses.map((address) => <div key={address.id} className="rounded-2xl border border-[#e6e0d5] bg-[#fffdf8] p-4">
              <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-extrabold">{address.title}</p><p className="mt-1 text-xs text-[#777970]">{address.recipientName} · <span dir="ltr">{address.phone}</span></p></div><button type="button" onClick={() => void removeAddress(address.id)} disabled={addressLoading} className="text-[11px] font-bold text-[#94675f] disabled:opacity-50">حذف</button></div>
              <p className="mt-3 text-xs leading-6 text-[#62685e]">{address.province}، {address.city}، {address.address}</p><p className="mt-1 text-[10px] text-[#99978f]">کد پستی: <span dir="ltr">{address.postalCode}</span></p>
            </div>) : <p className="rounded-2xl bg-[#f5f1e9] p-4 text-xs leading-6 text-[#777970]">هنوز آدرسی ثبت نکرده‌اید. برای ادامه سریع‌تر در Checkout یک آدرس اضافه کنید.</p>}
          </div>
        </section>

...
export default function MobileMenu() {
  ...
  return <><button type="button" onClick={() => setOpen(true)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[#d8d2c7] bg-white/65 text-[#334238] transition hover:bg-white lg:hidden" aria-label="باز کردن منوی سایت" aria-expanded={open} aria-haspopup="dialog"><span className="flex w-5 flex-col gap-1.5" aria-hidden="true"><span className="h-px w-full bg-current" /><span className="h-px w-3/4 bg-current" /><span className="h-px w-full bg-current" /></span></button>{typeof document !== "undefined" && menu ? createPortal(menu, document.body) : null}</>;
}

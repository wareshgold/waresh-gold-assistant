...
const priceBands = [
  ...
  { label: "۱۰ تا ۲۰ میلیون", min: 10_000_000, max: 20_000_000 },
  { label: "۲۰ تا ۳۰ میلیون", min: 20_000_000, max: 30_000_000 },
  { label: "بیشتر از ۳۰ میلیون", min: 30_000_000, max: Number.POSITIVE_INFINITY },
];
      ...
      <div className="mb-10 flex flex-col gap-5 border-y border-[#dcd8cd] py-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-bold transition ${category === "all" ? "bg-[#25392f] text-white" : "bg-white/70 text-[#686c64] hover:bg-white"}`}
          >
            همه
          </button>
          {PRODUCT_CATEGORIES.map((item) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-bold transition ${category === item.value ? "bg-[#25392f] text-white" : "bg-white/70 text-[#686c64] hover:bg-white"}`}
            >
              {item.label}
            </button>
          ))}
        ...
        <select
          ...
          className="min-w-48 border border-[#d8d1c5] bg-[#fffdf8] px-4 py-2.5 text-sm font-semibold text-[#4f514a] outline-none focus:border-[#a47d3f]"
          ...
          {products.map((product) => (
            <article key={product.id} className="group overflow-hidden rounded-[2rem] border border-[#e2ddd3] bg-[#fffdf8] shadow-[0_14px_45px_rgba(55,52,43,0.06)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(55,52,43,0.11)]">
              <div className="relative flex h-64 items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_50%_38%,#f3ead5_0%,#e8e3d8_48%,#d6ddd3_100%)]">
                ...
                <div className="relative flex h-36 w-36 items-center justify-center rounded-full border border-[#b28b4c]/35 bg-[#f8f3e7]/70 text-7xl text-[#a47d3f] shadow-[0_20px_50px_rgba(80,68,43,0.12)] transition duration-700 group-hover:scale-105 group-hover:rotate-3">
                  {product.icon}
                </div>
                <span className="absolute right-5 top-5 rounded-full border border-white/70 bg-white/65 px-3 py-1 text-[11px] font-bold text-[#746a5d] backdrop-blur">
